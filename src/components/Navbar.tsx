...
const Navbar = ({ onLogoClick, onCollaborationClick, onBlogClick }: { onLogoClick: () => void, onCollaborationClick?: (slug: string) => void, onBlogClick: () => void }) => {
    ...
    return (
        <div className="fixed top-0 left-0 right-0 z-50 flex flex-col font-sans">
            {/* Top Promo Bar */}
            <div className="bg-promo text-[#231F20]/80 text-[10px] md:text-[11px] font-medium tracking-widest uppercase py-2.5 text-center px-4 w-full">
            ...
            <nav className="bg-background border-b border-border/40 py-5 px-6 md:px-12 transition-colors duration-300">
                <div className="container mx-auto flex items-center justify-between">
                    ...
                    {/* Mobile Menu Button */}
                    <button
                        className="lg:hidden text-[#231F20] hover:opacity-70 transition-opacity"
                    ...
                    <div onClick={onLogoClick} className="flex flex-col items-center group cursor-pointer justify-center relative">
                        ...
                        />
                        <span className="font-uncial text-[0.96rem] tracking-[-0.02em] uppercase opacity-90 mt-[-2px] text-[#130F10]" style={{ transform: "scaleX(0.75)" }}>TARRASQUE</span>
                    </div>
                        ...
                        {navLinks.map((link) => (
                            <div key={link.name} className="relative group h-full flex items-center">
                                <a
                                    ...
                                    className="flex items-center gap-1.5 text-[11px] xl:text-xs font-light uppercase tracking-widest text-[#6B6869] hover:text-[#231F20] transition-colors py-4 relative"
                                >
                                    {link.name}
                                    <span className="absolute bottom-2 left-0 w-0 h-[0.5px] bg-[#6B6869] transition-all duration-300 group-hover:w-full"></span>
                                    {link.hasDropdown && (
                                        <ChevronDown size={10} className="text-muted group-hover:text-[#6B6869] transition-colors" />
                                    )}
                                </a>
                                        ...
                                        {link.items.map((item) => {
                                            ...
                                            return (
                                                <a
                                                    ...
                                                    className="text-[13px] text-[#231F20]/80 hover:text-[#231F20] hover:bg-[#231F20]/5 py-2.5 px-6 text-left transition-colors font-sans normal-case tracking-normal block"
                                                >
                                                    {label}
                                                </a>
                                            )
                    ...
                    {/* Right Icons */}
                    <div className="flex items-center gap-5 md:gap-6 text-[#231F20]">
                        ...
                        <div className="absolute top-full left-0 w-full bg-background border-b border-border p-6 lg:hidden flex flex-col gap-4 shadow-xl animate-in slide-in-from-top-2 h-[100vh]">
                            {navLinks.map((link) => (
                                <a
                                    key={link.name}
                                    href={link.href}
                                    className="flex items-center justify-between text-sm uppercase tracking-widest font-light text-[#6B6869] hover:text-[#231F20] py-3 border-b border-border/20"
                                    onClick={(e) => {
